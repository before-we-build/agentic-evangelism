...
Read [references/storyboard.md](references/storyboard.md) to identify subideas, plan approximate scene durations, and write `storyboard.json`. Keep repeated sections in performance order and distinguish lyric content from your visual interpretation. Embedded lyrics can differ from what was actually sung. In this no-alignment workflow, scene times are an editorial estimate, **not verified word, line, or chorus timestamps**. Explain this once to the user.

## Prepare images

If the host exposes a built-in image generation tool, use it with the available `imagegen` skill. That tool is a separate host capability; installing this skill does not enable it. If unavailable or persistently failing, explain the limitation and obtain explicit consent before a separate API/CLI workflow, installation, or potentially paid fallback. User-supplied images are another option. Do not request API secrets in chat. Create at least one distinct image per main subidea; long subideas may receive several related views. Keep one coherent visual style and reuse the relevant motif for repeated choruses. Every scene must point to a saved image, and every main subidea must have an image. Do not fall back silently to one image for the full song if generation is incomplete.
...
After planning the storyboard and shared visual style, launch independent image requests concurrently through the built-in tool, with one call per distinct asset. For a small set such as six images, submit the independent calls together when the host permits; use bounded batches for larger sets or service limits. Parallel tool calls do not require subagents or the CLI fallback. If an image depends on a generated reference, generate and inspect that reference first, then parallelize its independent follow-ups.

Assign each request a stable asset/subidea ID before launch and retain its exact prompt. In code mode, await all launched calls with `Promise.allSettled` (or an equivalent supported mechanism), inspect every outcome, and map saved paths by ID rather than completion order. Report individual completions as they arrive; keep base64 image payloads out of text logs and use the image rendering helper for visual inspection. Preserve successful images if another request fails; retry only failed requests when appropriate, reducing concurrency if throttled, and follow the imagegen fallback rules for persistent failures. Build the video only after every required asset has been saved and inspected and the storyboard paths have been checked.
