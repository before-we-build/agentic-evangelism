...
Apply constraints based on the active scene policy: use explicit positive physical descriptions (`plain plaster walls`, `ordinary sweater`) combined with targeted exclusions (`no halos, no devotional icons, no onion domes, no unrequested vestments`). For `interdenominational_unity` scenes, preserve deliberately requested traditional attire (e.g. simple cassock or clerical collar) while strictly excluding halos and icon veneration. Request portrait 9:16 art grounded in the selected lyrics, with important details clear of bottom/right overlays and no text unless requested. Inspect each image against the active visual policy. For supplied local images, view them first. Preserve the actual prompts and accessible artwork copies alongside the storyboard. Explain that pictures change through the song but are individually still. Follow the imagegen skill's fallback requirements if generation fails.
