...
Create a distinct image per main subidea; alternate related views for long passages. For a full-song TikTok slideshow made from stills, start with a visual cadence of roughly **one distinct image every 8–12 seconds**. Add genuinely different related shots at musically active transitions rather than holding one frame for a long stretch, artificially zooming it, or duplicating it. Treat this range as an editorial starting point: slow, contemplative passages may hold a frame longer when that supports the song. Assemble prompts using the modular formula:
